...
- Name: voice-input.spec.ts >> Voice Input Feature >> should allow fallback to typing when voice is not available
- Location: tests\e2e\voice-input.spec.ts:101:7
...
```ts
  2   | 
  3   | test.describe('Voice Input Feature', () => {
  ...
  18  |   });
  19  | 
  20  |   test('should detect voice support and show button on Chrome', async ({ page, browserName }) => {
