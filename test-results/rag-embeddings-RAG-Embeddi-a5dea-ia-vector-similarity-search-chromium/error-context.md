...
- Name: rag-embeddings.spec.ts >> RAG Embedding System >> should retrieve documents via vector similarity search
- Location: tests\e2e\rag-embeddings.spec.ts:50:7
...
```ts
  ...
  3   | 
  4   | const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
  5   | 
  6   | test.describe('RAG Embedding System', () => {
